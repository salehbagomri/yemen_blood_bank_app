...
عند إصدار رسمي فقط:
1. زيادة الإصدار في `pubspec.yaml` (وهو مصدر الحقيقة الوحيد للإصدار — بصيغة `X.Y.Z+buildNumber`).
2. تشغيل `flutter analyze` والتأكد من نظافة كاملة.
3. `flutter build appbundle --release` بعد الاجتياز.
4. التحقق من بصمات التوقيع (موثقة في handoff قسم د) عبر `keytool` أو `apksigner`.
